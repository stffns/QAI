    ...
    try:
        ...
        with Session(engine) as session:
            ...
            result = session.exec(
                ...
                .join(AppEnvironmentCountryMapping, ApplicationEndpoint.mapping_id == AppEnvironmentCountryMapping.id)
                .join(Apps, AppEnvironmentCountryMapping.application_id == Apps.id)
                .join(Environments, AppEnvironmentCountryMapping.environment_id == Environments.id)
                .join(Countries, AppEnvironmentCountryMapping.country_id == Countries.id)
                .limit(5)
            ).all()

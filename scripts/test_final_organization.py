...
def test_clean_organization():
    ...
    try:
        ...
        repo_methods = [m for m in dir(ApplicationEndpointRepository) if not m.startswith('_')]
        expected_methods = ['create_endpoint', 'get_by_mapping_id', 'get_all', 'get_by_id']
    ...
    try:
        ...
        if db_path.exists():
            ...
            print(f"✅ Database migration confirmed: {count} endpoints with clean structure")
            success_count += 1
        else:

...
def test_clean_organization():
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            # Test search
            search_results = endpoints_repo.search_endpoints(
                search_term="test", is_active=True
            )
            ...
            if endpoint_count > 0:
                ...
                mapping_endpoints = endpoints_repo.get_by_mapping(
                    first_endpoint.mapping_id
                )
                print(
                    f"📊 Endpoints for mapping {first_endpoint.mapping_id}: {len(mapping_endpoints)}"
                )
        ...
        has_clean_structure = (
            "mapping_id" in model_fields
            and "application_id" not in model_fields
            and "environment_id" not in model_fields
            and "country_id" not in model_fields
        )
...
def test_import_compatibility():
    ...
    try:
        ...
        try:
            from database.models.application_endpoints_clean import (

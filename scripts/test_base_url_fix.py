...
def test_postman_collection_import():
    ...
    test_collection = {
        "info": {"name": "Test API with Base URLs", "version": "1.0.0"},
        "item": [
            {
            ...
            },
            {
                ...
                "request": {
                    ...
                    "body": {"mode": "raw", "raw": '{"name": "Updated User"}'},
                },
            },
        ],
    }
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            for endpoint in endpoints:
                ...
                print(
                    f"    URL: {endpoint.endpoint_url}"
                )  # Should be just path, not full URL
                print(f"    Method: {endpoint.http_method}")

                # Verify it's just a path, not full URL
                if (
                    endpoint.endpoint_url.startswith("/")
                    and "://" not in endpoint.endpoint_url
                ):

...
class ServiceManager:
                    ...
                    for pid in pids:
                        if pid.strip():
                            try:
                                ...
                                print(
                                    f"🔪 Killing process {process.name()} (PID: {pid.strip()}) on port {port}"
                                )
                                process.terminate()
                                ...
                                try:
                                    ...
                                    print(
                                        f"💀 Force killed PID {pid.strip()} on port {port}"
                                    )
                                    return True
                                except:
                                    pass
    ...
    def start_service(
        self, name: str, command: list, cwd: Path | None = None
    ...
    def wait_for_service(
        self, name: str, url: str, timeout: int = 30, is_websocket: bool = False
    ):
    ...
    def start_all_services(self):
        ...
        api_process = self.start_service(
            "REST API", [sys.executable, "src/api/metrics_api.py"]
        )
        if api_process:
        ...
        if (
            ready_services >= 3
        ):  # At least 3 services (WebSocket might not respond to HTTP)
            ...
            while self.running:
                ...
                for name, process in self.services:
                    if process.poll() is not None:
                        print(
                            f"❌ {name} has stopped unexpectedly (exit code: {process.returncode})"
                        )
                        if name in [
                            "Prometheus Exporter",
                            "REST API",
                            ...
                            print(
                                f"⚠️  {name} failed but continuing with other services"
                            )

...
class PerformanceCleanupManager:
    ...
    def run_cleanup(self, keep_recent_count: int = 1, retention_days: int = 7) -> dict:
        ...
        print(
            f"📋 Strategy: Keep {keep_recent_count} recent, archive others, {retention_days}-day retention"
        )
        print()
...
def main():
    ...
    results = cleanup_manager.run_cleanup(
        keep_recent_count=args.keep_recent, retention_days=args.retention_days
    )

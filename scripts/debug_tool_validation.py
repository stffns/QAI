...
def debug_tool_validation():
    ...
    try:
        ...
        print(
            f"Tool name: {database_stats.name if hasattr(database_stats, 'name') else 'No name'}"
        )
        ...
        print(
            f"Run is callable: {hasattr(database_stats, 'run') and callable(getattr(database_stats, 'run'))}"
        )
        ...
        tool_methods = [
            attr for attr in dir(database_stats) if not attr.startswith("_")
        ]

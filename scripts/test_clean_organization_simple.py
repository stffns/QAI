...
def test_model_structure():
    ...
    try:
        ...
        has_old_fields = any(field in model_fields for field in ['application_id', 'environment_id', 'country_id'])
        ...
        if has_mapping_id and not has_old_fields:
            ...
            print(f"Old fields found: {[f for f in ['application_id', 'environment_id', 'country_id'] if f in model_fields]}")
        else:
...
def test_repository_structure():
    ...
    try:
        ...
        repo_methods = [method for method in dir(ApplicationEndpointRepository) 
                       if not method.startswith('_') and callable(getattr(ApplicationEndpointRepository, method))]
        ...
        missing_methods = [method for method in expected_methods if method not in repo_methods]
...
def test_imports_cleanup():
    ...
    try:
        ...
        try:
            from database.models.application_endpoints_old import ApplicationEndpoint as OldEndpoint

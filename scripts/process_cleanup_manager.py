...
class ProcessCleanupManager:
    ...
    def __init__(self):
        ...
        try:
            if self.settings.database.url:
                self.uow_factory = create_unit_of_work_factory(
            ...
            with self.uow_factory.create_scope() as uow:
                ...
                for status in [ExecutionStatus.PENDING, ExecutionStatus.RUNNING]:
                    ...
                    for execution in executions:
                        ...
                        if execution.created_at:
                            age_minutes = (
                                current_time - execution.created_at
                            ).total_seconds() / 60
                    ...
                    for execution in executions:
                        try:
                            if (
                                ...
                                and execution.created_at < stuck_threshold
                            ):
    ...
    def kill_zombie_processes(self, dry_run: bool = True) -> Dict[str, Any]:
        ...
        result = {"killed": 0, "errors": 0, "processes_killed": [], "dry_run": dry_run}
    ...
    async def generate_health_report(self) -> Dict[str, Any]:
        ...
        if report["execution_analysis"].get("total_stuck", 0) > 0:
            report["recommendations"].append(
                "🧹 Consider running cleanup to remove stuck executions"
            )

        if (
            len([p for p in report["qa_processes"] if p.get("age_minutes", 0) > 120])
            > 2
        ):
            report["recommendations"].append(
                "🔄 Multiple old processes detected - consider restarting services"
            )
...
async def main():
    ...
    parser.add_argument(
        "--action",
        choices=["report", "cleanup", "kill-zombies", "auto"],
        default="report",
    ...
    if args.action == "report":
        ...
        if args.json:
            ...
            for proc in report["qa_processes"]:
                status_emoji = (
                    "🟢"
                    if proc.get("age_minutes", 0) < 30
                    else "🟡" if proc.get("age_minutes", 0) < 120 else "🔴"
                )
                print(
                    f"  {status_emoji} PID {proc['pid']}: {proc.get('name', 'unknown')} ({proc.get('age_minutes', 0):.1f}m)"
                )
            ...
            if not exec_analysis.get("error"):
                print(f"\n📋 Executions:")
                print(
                    f"  🔄 Running: {len(exec_analysis.get('running_executions', []))}"
                )
                print(
                    f"  ⏳ Pending: {len(exec_analysis.get('pending_executions', []))}"
                )
        ...
        if args.json:
            ...
            if result.get("error"):
                ...
                for exec_info in result["executions_cleaned"]:
                    print(
                        f"  🧹 Execution {exec_info['id']}: {exec_info['original_status']} -> FAILED ({exec_info['age_hours']:.1f}h old)"
                    )
        ...
        if args.json:
            ...
            if result.get("error"):
                ...
                for proc_info in result["processes_killed"]:
                    print(
                        f"  ☠️ PID {proc_info['pid']}: {proc_info['name']} - {proc_info['reason']}"
                    )
        ...
        if report["execution_analysis"].get("total_stuck", 0) > 0:
            print(
                f"🧹 Found {report['execution_analysis']['total_stuck']} stuck executions, cleaning up..."
            )

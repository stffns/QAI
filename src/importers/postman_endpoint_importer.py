...
class PostmanEndpointImporter:
    ...
    def import_collection(
        ...
        if application.id is None or environment.id is None or country.id is None:
            return {"error": "Missing IDs for application, environment, or country"}
    ...
    def _get_or_create_application(self, app_code: str) -> Optional[Apps]:
        ...
        app = self.session.exec(select(Apps).where(Apps.app_code == app_code)).first()
    ...
    def _get_or_create_mapping(
        self, app_id: int, env_id: int, country_id: int, env_vars: Dict[str, str]
    ) -> AppEnvironmentCountryMapping:
        ...
        if not mapping:
            ...
            base_url = env_vars.get(
                "BASE_URL", env_vars.get("baseUrl", "https://api.example.com")
            )

            # Create new mapping with headers from environment
            default_headers = {}
            for key, value in env_vars.items():
                if "header" in key.lower() or key.lower() in [
                    "authorization",
                    "content-type",
                    "accept",
                ]:
    ...
    def _extract_endpoint_data(
        self, item: Dict, env_vars: Dict[str, str]
    ) -> Dict[str, Any]:
    ...
    def _normalize_path(self, raw_path: str, env_vars: Dict[str, str]) -> str:
        ...
        if "://" in normalized:
            ...
            normalized = re.sub(
                r"/-+/", "/", normalized
            )  # Remove orphaned dashes between slashes
                        ...
                        for header in headers:
                            if isinstance(header, dict):
                                self._extract_variables_from_text(
                                    header.get("key", ""), extracted_variables
                                )
                                self._extract_variables_from_text(
                                    header.get("value", ""), extracted_variables
                                )
    ...
    def _extract_variables_from_text(
        self, text: str, variables_dict: Dict[str, str]
    ) -> None:

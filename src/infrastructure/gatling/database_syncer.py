    ...
    def sync_once(self) -> dict:
        ...
        try:
            with self.uow_factory.create_scope() as uow:
                ...
                logger.debug(f"Found {len(pending_executions)} pending DB executions, {len(memory_statuses)} in memory")
                        ...
                        if new_status != db_execution.status:
                            ...
                            if new_status in [ExecutionStatus.COMPLETED, ExecutionStatus.FAILED, ExecutionStatus.CANCELLED]:
                                if not db_execution.end_time:
                                    end_time = memory_run.finished_at or datetime.utcnow()
                            ...
                            if new_status == ExecutionStatus.RUNNING and not db_execution.start_time:
                                start_time = memory_run.started_at or datetime.utcnow()
                                exec_repo.update_metrics(execution_id, {"start_time": start_time})
                            
                            stats["synced_count"] += 1
                            
                            logger.info(f"Synced execution {execution_id[:8]}... from {db_execution.status} to {new_status}")
                    else:
                        # Execution not found in memory - check if completed or stuck
                        age_minutes = (datetime.utcnow() - db_execution.created_at).total_seconds() / 60
                        if age_minutes > 10:  # Consider for processing after 10 minutes
                            # Check if execution actually completed by looking for results
                            completed_status = self._check_execution_completion(execution_id)
                            ...
                            if completed_status:
                                ...
                                try:
                                    metrics = self._extract_completion_metrics(execution_id)
                                    if metrics:
                                        exec_repo.update_metrics(execution_id, metrics)
                                except Exception as e:
                                    logger.warning(f"Could not extract metrics for {execution_id[:8]}...: {e}")
                                
                                stats["synced_count"] += 1
                                logger.info(f"Marked completed execution {execution_id[:8]}... as COMPLETED after {age_minutes:.1f} minutes")
                            else:
                                ...
                                # Add failure note
                                exec_repo.update_metrics(execution_id, {
                                    "execution_notes": f"Failed: execution not found in memory store and no results after {age_minutes:.1f} minutes"
                                })
                                stats["synced_count"] += 1
                                logger.warning(f"Marked stuck execution {execution_id[:8]}... as FAILED after {age_minutes:.1f} minutes")
        ...
        while self.running:
            try:
                ...
                if stats["synced_count"] > 0 or stats["error_count"] > 0:
                    logger.info(f"Sync cycle: {stats['synced_count']} synced, {stats['error_count']} errors, "
        ...
        try:
            ...
            if summary_file.exists():
                ...
                try:
                    ...
                    return (summary.get("parsed", False) and 
                           isinstance(total, (int, float)) and
                           total > 0)
    ...
    def _extract_completion_metrics(self, execution_id: str) -> dict:
        ...
        try:
            ...
            if summary.get("error_rate") is not None:
                metrics["error_rate"] = float(summary["error_rate"]) / 100.0  # Convert percentage to ratio
            elif "global_stats" in summary and summary["global_stats"].get("error_rate") is not None:
                metrics["error_rate"] = float(summary["global_stats"]["error_rate"]) / 100.0

...
def _parse_from_html_report(html_path: Path) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for row in rows:
            ...
            name_match = re.search(r'<span[^>]*class="ellipsed-name"[^>]*>([^<]+)</span>', row)
            if not name_match:
                continue
            ...
            col_values = re.findall(r'<td[^>]*class="value[^"]*col-(\d+)"[^>]*>([^<]+)</td>', row)
...
def _parse_from_runner_log(results_root: Path) -> Dict[str, Any]:
    ...
    log_path = (results_root if results_root.is_dir() else results_root.parent) / "runner.log"
    if not log_path.exists():
        return {"parsed": False, "reason": "runner.log not found", "path": str(results_root)}
    try:
        text = log_path.read_text(encoding="utf-8", errors="ignore")
        # Extract metrics from the "Global Information" block
        req_m = re.search(r"request count\s*\|\s*([0-9,]+)\s*\|\s*([0-9,]+)\s*\|\s*([0-9,\-]+)", text)
        ...
        found_any = any(v is not None for k, v in summary.items() if k not in ("parsed", "path"))
        if not found_any:
            return {"parsed": False, "reason": "runner.log unparseable", "path": str(results_root)}
        return summary
    except Exception as e:
        return {"parsed": False, "reason": f"runner.log parse error: {e}", "path": str(results_root)}

...
class QAOAuthTool:
        ...
        try:
            ...
            if not config:
                return {
                    ...
                    "available_configs": [
                        ...
                        for c in self.list_configurations()
                        if c["is_complete"]
                    ],
                }
            ...
            return {
                ...
                "configuration": {
                    ...
                    "client_id": config["client"]["client_id"][:20]
    ...
    def _generate_oauth_token(self, config: Dict) -> Dict:
        ...
        # Header JWT
        header = {"alg": "ES256", "typ": "JWT", "kid": config["jwk"]["jwk_data"]["kid"]}

        # Firmar el token
        access_token = jws.sign(
            payload, config["jwk"]["jwk_data"], algorithm="ES256", headers=header
        )
...
def main():
    ...
    if len(sys.argv) == 1 or "--help" in sys.argv:
        ...
        print("Uso:")
        print(
            "  python qa_oauth_tool.py --list                    # Listar configuraciones"
        )
        print(
            "  python qa_oauth_tool.py EVA STA RO              # Generar token específico"
        )
        ...
        if "--list" in sys.argv:
            ...
            if complete_configs:
                ...
                for config in complete_configs:
                    print(
                        f"   {config['app_code']} {config['env_code']} {config['country_code']} - {config['app_name']} | {config['env_name']} | {config['country_name']}"
                    )
            ...
            if incomplete_configs:
                ...
                for config in incomplete_configs:
                    ...
                    print(
                        f"   {config['app_code']} {config['env_code']} {config['country_code']} - Faltan: {', '.join(missing)}"
                    )

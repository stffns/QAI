...
def get_mapping_for_oauth_user(conn: sqlite3.Connection, environment: str, product: str, country_code: str):
    ...
    WHERE em.env_code = ?
      ...
      AND cm.country_code = ?
    """, (environment.upper(), f'%{product.upper()}%', f'%{product.upper()}%', country_code.upper()))
    ...
    cursor.execute("SELECT id FROM apps_master WHERE app_code LIKE ? OR app_name LIKE ?", 
    ...
    cursor.execute("SELECT id FROM environments_master WHERE env_code = ?", (environment.upper(),))
    ...
    cursor.execute("SELECT id FROM countries_master WHERE country_code = ?", (country_code.upper(),))
    ...
    print(f"✅ Mapping creado: {product}/{environment}/{country_code} (ID: {mapping_id})")
...
def migrate_oauth_users_table(conn: sqlite3.Connection):
    ...
    cursor.execute("SELECT id, email, environment, product, country_code FROM oauth_users")
    ...
    for user_id, email, environment, product, country_code in existing_users:
        ...
        mapping_id = get_mapping_for_oauth_user(conn, environment, product, country_code)
...
def migrate_oauth_jwks_table(conn: sqlite3.Connection):
    ...
    cursor.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='oauth_jwks'")
...
def migrate_oauth_app_clients_table(conn: sqlite3.Connection):
    ...
    # Verificar si la tabla existe
    cursor.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='oauth_app_clients'")
...
def validate_integration(conn: sqlite3.Connection):
    ...
    for user_mapping in user_mappings:
        print(f"      • {user_mapping[0]} → {user_mapping[1]}/{user_mapping[2]}/{user_mapping[3]}")

    ...
    def get_config(
        ...
        if not user:
            raise EntityNotFoundError(
                "OAuthUser",
                "config",
                f"{environment}/{country_code}/{product}"
            )
...
class OAuthTokenService:
    ...
    async def get_access_token(
        ...
        provider_metadata = await self.provider_service.get_provider_metadata(config.issuer)
        
        # 3. Generar código PKCE
        code_verifier = PKCEService.generate_code_verifier()
        
        # 4. Generar URL de autorización
        auth_url = self.provider_service.generate_authorization_url(config, code_verifier)

...
class OAuthTokenTool:
    ...
    def generate_token(self, app_code: str, env_code: str, country_code: str) -> Dict[str, Any]:
        """
    ...
    def validate_configuration(self, app_code: str, env_code: str, country_code: str) -> Dict[str, Any]:
        """
    ...
    def get_gatling_authorization_header(self, app_code: str, env_code: str, country_code: str) -> str:
        """
...
def main():
    ...
    if sys.argv[1] == '--list':
        ...
        if incomplete_configs:
            ...
            for config in incomplete_configs:
                print(f"   {config['config_key']} - {config['description']}")
                missing = []
                if not config['user_email']: missing.append('usuario')
                if not config['client_id']: missing.append('client')
                if not config['jwk_key_id']: missing.append('jwk')
    ...
    if len(sys.argv) == 4:
        ...
        if not validation['config_exists']:
            print(f"❌ No existe configuración para {app_code} {env_code} {country_code}")
            return

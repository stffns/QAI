...
class IntegratedOAuthTokenGenerator:
    ...
    def generate_token(self) -> dict:
        ...
        print(
            f"🚀 Generando token para {self.config['app_code']} {self.config['env_code']} {self.config['country_code']}"
        )
    ...
    def _generate_mock_auth_code(self, code_verifier: str, state: str) -> str:
        ...
        # Crear header
        header = {"alg": "ES256", "typ": "JWT", "kid": jwk_data["kid"]}
    ...
    def _exchange_code_for_token(self, auth_code: str) -> dict:
        ...
        header_data = json.loads(
            base64.urlsafe_b64decode(auth_code.split(".")[0] + "==").decode()
        )
        payload_data = json.loads(
            base64.urlsafe_b64decode(auth_code.split(".")[1] + "==").decode()
        )
    ...
    def _validate_token(self, access_token: str) -> dict:
        ...
        try:
            ...
            header_data = json.loads(
                base64.urlsafe_b64decode(access_token.split(".")[0] + "==").decode()
            )
            payload_data = json.loads(
                base64.urlsafe_b64decode(access_token.split(".")[1] + "==").decode()
            )
    ...
    try:
        ...
        if "--list-configs" in sys.argv or "--list" in sys.argv:
            ...
            for config in configs:
                status = (
                    "✅ COMPLETA" if config["user_count"] > 0 else "⚠️  SIN USUARIOS"
                )
                print(
                    f"{config['app_code']} {config['env_code']} {config['country_code']} - {status}"
                )
                print(f"   App: {config['app_name']}")
                print(f"   Env: {config['env_name']}")
                print(f"   Country: {config['country_name']}")
                print(
                    f"   Usuarios: {config['user_count']} | JWKs: {config['active_jwks']} | Clients: {config['active_clients']}"
                )
                print()
        ...
        if not config:
            print(
                f"❌ No se encontró configuración para {app_code} {env_code} {country_code}"
            )
        ...
        if missing_components:
            print(
                f"❌ Configuración incompleta, faltan: {', '.join(missing_components)}"
            )

...
class WebSocketHealthIntegration:
    ...
    def __init__(self):
        ...
        self.notification_cooldown = (
            300  # 5 minutos entre notificaciones del mismo tipo
        )
        ...
        try:
            ...
            if current_status != self.last_status:
                ...
                if (
                    ...
                    or now - self.last_notification > self.notification_cooldown
                ):
                    ...
                    notification = {
                        ...
                        "action_required": current_status
                        in [ServiceStatus.DEGRADED, ServiceStatus.OUTAGE],
                    }
...
async def add_health_monitoring_to_message(message: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if (
            ...
            or status_alert is not None
            or "error" in message.get("content", "").lower()
        ):
            ...
            message["health_context"] = {
                ...
                "possible_service_issue": health_status["openai_status"]["status"]
                != "healthy",

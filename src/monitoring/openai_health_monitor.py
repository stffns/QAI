...
class OpenAIHealthMonitor:
            ...
            if self.health_file.exists():
                with open(self.health_file, "r") as f:
                    ...
                    for check_data in history_data:
                        ...
                        if "status" in check_data and isinstance(
                            check_data["status"], str
                        ):
    ...
    def _save_health_data(self):
        ...
        try:
            ...
            recent_history = [
                check for check in self.health_history if check.timestamp >= cutoff_str
            ]
            ...
            if not api_key:
                ...
                try:
                    ...
                    config_path = os.path.join(
                        os.path.dirname(__file__), "..", "..", "agent_config.yaml"
                    )
    ...
    def _update_status(self, check: HealthCheck):
        ...
        if previous_status != self.current_status:
            ...
            logger.warning(
                f"OpenAI status changed: {previous_status.value} → {self.current_status.value}"
            )
    ...
    def get_health_summary(self) -> ServiceHealth:
        ...
        return ServiceHealth(
            ...
            last_checked=(
                self.health_history[-1].timestamp if self.health_history else "Never"
            ),
    ...
    async def run_continuous_monitoring(self):
        ...
        while True:
            try:
                ...
                logger.info(
                    f"{emoji} OpenAI Health: {check.status.value} ({check.response_time:.2f}s)"
                )

    ...
    for line in lines:
        ...
        if not line or line.startswith("#"):
            if line.startswith("# HELP"):
                ...
                if len(parts) >= 3:
                    current_metric_help = (
                        parts[2] if len(parts) == 3 else " ".join(parts[3:])
                    )
...
async def get_executions_summary():
    ...
    try:
        ...
        def extract_app_info_with_repos(
            execution_name: str, execution_environment: str, uow
        ):
                ...
                if app:
                    ...
                    if execution_environment:
                        try:
                            ...
                            mappings = (
                                uow.app_environment_country_mappings.get_by_application(
                                    app.app_code
                                )
                            )
                            if mappings:
                                ...
                                for mapping in mappings:
                                    env = uow.environments.get_by_id(
                                        mapping.environment_id
                                    )
                                    if (
                                        env
                                        and env.environment_code
                                ...
                                if selected_mapping:
                                    country = uow.countries.get_by_id(
                                        selected_mapping.country_id
                                    )
                                    if country:
        ...
        with uow_factory.create_scope() as uow:
            ...
            for status in ExecutionStatus:
                count_stmt = (
                    select(func.count())
                    .select_from(PerformanceTestExecution)
            ...
            return {
                "summary": {
                    ...
                    "success_rate": (
                        (completed / total_tests * 100) if total_tests > 0 else 0
                    ),
...
async def get_endpoints_summary():
    ...
    try:
        ...
        from database.connection import db_manager
        from database.models.performance_endpoint_results import (
        ...
        with uow_factory.create_scope() as uow:
            ...
            endpoint_stmt = (
                select(PerformanceEndpointResults)
                .order_by(desc(PerformanceEndpointResults.created_at))
                .limit(50)
            )
            ...
            for endpoint_result in endpoint_results:
                ...
                endpoint_data["metrics"]["total_requests"] += (
                    getattr(endpoint_result, "total_requests", 0) or 0
                )
                endpoint_data["metrics"]["successful_requests"] += (
                    getattr(endpoint_result, "successful_requests", 0) or 0
                )
                endpoint_data["metrics"]["failed_requests"] += (
                    getattr(endpoint_result, "failed_requests", 0) or 0
                )
                ...
                if getattr(endpoint_result, "avg_response_time", None):
                    ...
                    endpoint_data["metrics"]["avg_response_time"] = (
                        current_avg + new_avg
                    ) / 2
                ...
                if getattr(endpoint_result, "p95_response_time", None):
                    ...
                    endpoint_data["metrics"]["p95_response_time"] = max(
                        current_p95, new_p95
                    )
                ...
                if getattr(endpoint_result, "error_rate", None):
                    ...
                    endpoint_data["metrics"]["error_rate"] = max(
                ...
                if app_info["app_code"]:
                    endpoint_data["apps"].add(
                        f"{app_info['app_code']} ({app_info['app_name']})"
                    )
                if app_info["country_code"]:
                    endpoint_data["countries"].add(
                        f"{app_info['country_code']} ({app_info['country_name']})"
                    )
                if getattr(execution, "execution_environment", None):
                    endpoint_data["environments"].add(
                        getattr(execution, "execution_environment")
                    )
                ...
                app_endpoint_breakdown[app_key]["total_requests"] += (
                    getattr(endpoint_result, "total_requests", 0) or 0
                )
                ...
                country_endpoint_breakdown[country_key]["endpoints_tested"].add(
                    endpoint_name
                )
                country_endpoint_breakdown[country_key]["total_requests"] += (
                    getattr(endpoint_result, "total_requests", 0) or 0
                )
            ...
            for endpoint_data in endpoints.values():
                ...
                endpoint_data["metrics"]["success_rate"] = (
    ...
    try:
                    ...
                    problems.append(
                        {
                            ...
                            "severity": (
                                "critical" if metric["value"] > 50 else "warning"
                            ),
                    ...
                    problems.append(
                        {
                            "type": "slow_response",
                            "severity": (
                                "critical" if metric["value"] > 2000 else "warning"
                            ),
                            "endpoint": metric["labels"].get("endpoint", "unknown"),
                            "method": metric["labels"].get("method", "unknown"),
                            "value": metric["value"],
        ...
        return {
            ...
            "critical_problems": len(
                [p for p in problems if p["severity"] == "critical"]
            ),
            "warning_problems": len(
...
async def get_endpoint_detail(endpoint_name: str, method: Optional[str] = None):
    ...
    try:
        ...
        endpoint_data = {"endpoint": endpoint_name, "method": method, "metrics": {}}
...
async def get_recent_executions(
    status: Optional[str] = None, environment: Optional[str] = None, limit: int = 10
):
...
async def get_execution_details(execution_id: str):
                    ...
                    try:
                        ...
                        if mappings:
                            ...
                            for mapping in mappings:
                                ...
                                if (
                                    env
                                    and env.environment_code == execution_environment
                                ):
                                    selected_mapping = mapping
                                    break
    ...
    try:
        with uow_factory.create_scope() as uow:
            # 1) Búsqueda exacta por execution_id (tal cual)
            execution_stmt = select(PerformanceTestExecution).where(
                PerformanceTestExecution.execution_id == execution_id
            )
            execution = uow.session.exec(execution_stmt).first()

            # 2) Fallback: búsqueda case-insensitive si no se encontró (ej: mayúsculas/minúsculas distintas)
            if not execution:
                try:
                    ci_stmt = select(PerformanceTestExecution).where(
                        func.lower(PerformanceTestExecution.execution_id)
                        == execution_id.lower()
                    )
                    execution = uow.session.exec(ci_stmt).first()
                    if execution:
                        logger.debug(
                            f"[details] Found execution by case-insensitive match: {execution.execution_id}"
                        )
                except Exception as _e:
                    logger.debug(f"[details] Case-insensitive lookup failed: {_e}")

            # 3) Fallback: si el parámetro parece un número, intentar por primary key 'id'
            if not execution and execution_id.isdigit():
                try:
                    pk_stmt = select(PerformanceTestExecution).where(
                        PerformanceTestExecution.id == int(execution_id)
                    )
                    execution = uow.session.exec(pk_stmt).first()
                    if execution:
                        logger.debug(
                            f"[details] Found execution by numeric primary key id={execution_id}"
                        )
                except Exception as _e:
                    logger.debug(f"[details] Numeric id lookup failed: {_e}")

            # 4) (Opcional) Fallback: intentar que coincida con execution_name exacto
            if not execution:
                try:
                    name_stmt = select(PerformanceTestExecution).where(
                        PerformanceTestExecution.execution_name == execution_id
                    )
                    execution = uow.session.exec(name_stmt).first()
                    if execution:
                        logger.debug(
                            f"[details] Found execution by execution_name match: {execution.execution_name}"
                        )
                except Exception as _e:
                    logger.debug(f"[details] execution_name lookup failed: {_e}")

            if not execution:
                logger.info(
                    f"[details] Execution not found with any strategy value={execution_id}"
                )
                raise HTTPException(
                ...
                endpoints_data.append(
                    {
                        ...
                        "performance": {
                            ...
                            or round(endpoint_error_rate, 2),
                            "performance_grade": getattr(
                                endpoint, "performance_grade", None
                            ),
                        },
                        "test_context": {
                            "concurrent_users": getattr(
                                endpoint, "concurrent_users", None
                            ),
                            "test_duration_seconds": getattr(
                                endpoint, "test_duration_seconds", None
                            ),
                        },
                    }
                )
            ...
            if total_endpoint_requests > 0:
                overall_endpoint_success_rate = (
                    total_endpoint_successes / total_endpoint_requests
                ) * 100
                overall_endpoint_error_rate = (
                    total_endpoint_failures / total_endpoint_requests
                ) * 100
            ...
            return {
                ...
                "execution": {
                    ...
                    "execution_name": getattr(execution, "execution_name", None)
                    or f"Test-{execution.id}",
                    "status": execution.status.value if execution.status else "UNKNOWN",
                    "execution_environment": getattr(
                        execution, "execution_environment", None
                    ),
                    "gatling_report_path": getattr(
                        execution, "gatling_report_path", None
                    ),
                    "app_name": app_info["app_name"],
                    "app_code": app_info["app_code"],
                    "country_name": app_info["country_name"],
                ...
                "endpoints_summary": {
                    ...
                    "overall_endpoint_success_rate": round(
                        overall_endpoint_success_rate, 2
                    ),
                    "overall_endpoint_error_rate": round(
                        overall_endpoint_error_rate, 2
                    ),
                    "execution_level_metrics": {
                        ...
                        or round(error_rate, 2),
                        "avg_response_time": getattr(
                            execution, "avg_response_time", None
                        ),
                        "p95_response_time": getattr(
                            execution, "p95_response_time", None
                        ),
                        "p99_response_time": getattr(
                            execution, "p99_response_time", None
                        ),

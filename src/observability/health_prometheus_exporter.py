    ...
    async def collect_openai_health_metrics(self):
        ...
        try:
            ...
            if result.get("last_check"):
                health_status["response_time"] = result["last_check"].get(
                    "response_time", 0
                )

            # Map status to numeric values
            status_mapping = {"healthy": 1, "degraded": 2, "outage": 3, "unknown": 0}
        ...
        try:
            ...
            for proc_info in qa_processes:
                try:
                    ...
                    for (
                        threshold_name,
                        threshold_minutes,
                    ) in self.process_thresholds.items():
                        if age_minutes > threshold_minutes:
                            key = f"{process_type}_{threshold_name}"
                            long_running_counts[key] = (
                                long_running_counts.get(key, 0) + 1
                            )
            ...
            for key, count in long_running_counts.items():
                ...
                LONG_RUNNING_PROCESSES.labels(
                    process_type=process_type, threshold_minutes=str(threshold_minutes)
                ).set(count)
                ...
                for status in [ExecutionStatus.PENDING, ExecutionStatus.RUNNING]:
                    ...
                    for category, count in age_counts.items():
                        STUCK_EXECUTIONS.labels(
                            status=status.value.lower(), age_category=category
                        ).set(count)
                    ...
                    for status in [ExecutionStatus.PENDING, ExecutionStatus.RUNNING]:
                        ...
                        for execution in executions:
                            try:
                                ...
                                error_count += 1
                                print(
                                    f"Error cleaning up execution {execution.id}: {e}"
                                )

            # Record cleanup metrics
            PROCESS_CLEANUP_OPERATIONS.labels(
                operation_type="stuck_execution_cleanup", success="true"
            ).inc(cleanup_count)

            if error_count > 0:
                PROCESS_CLEANUP_OPERATIONS.labels(
                    operation_type="stuck_execution_cleanup", success="false"
                ).inc(error_count)
        ...
        while True:
            try:
                ...
                if int(time.time()) % 1800 == 0:  # Every 30 minutes
                    cleanup_count, error_count = collector.cleanup_stuck_processes(
                        uow_factory
                    )
                    if cleanup_count > 0:

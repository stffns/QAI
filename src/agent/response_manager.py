...
class ResponseStrategy:
    ...
    def __init__(
        self, name: str, max_tokens: int, temperature: float, system_prompt: str
    ):
        self.name = name
...
- Balance brevity with completeness""",
        ...
        logger.debug(
            f"Loaded {len(self.strategies)} response strategies with cost optimization"
        )

    def analyze_message(
        self, user_message: str, session_history: Optional[List[str]] = None
    ) -> ResponseContext:
        """
        ...
        with LogStep("Analyzing message for response strategy", "ResponseManager"):
            ...
            if is_simple and self.reasoning_config.get("cost_optimization", True):
                ...
                concise_mode = self._should_use_concise_mode(
                    user_message, requires_reasoning
                )
            ...
            context.recommended_model = self._get_recommended_model(
                strategy_name, is_simple, requires_reasoning
            )
    ...
    def _get_recommended_model(
        self, strategy_name: str, is_simple: bool, requires_reasoning: bool
    ) -> str:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "models_configured": {
                "default": "gpt-5-mini",
                "reasoning": self.reasoning_config.get("reasoning_model", {}).get(
                    "id", "gpt-5"
                ),
                "simple": self.reasoning_config.get("simple_model", {}).get(
                    "id", "gpt-5-nano"
                ),
            },
        }

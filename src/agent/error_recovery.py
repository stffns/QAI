...
class ErrorRecoveryManager:
    ...
    def register_component(self, 
        ...
        logger.debug(f"Registered component {component_name} with {recovery_config.strategy.value} recovery")
    ...
    def execute_with_recovery(self,
        ...
        if config.strategy == RecoveryStrategy.RETRY:
            return self._execute_with_retry(component_name, operation, config, *args, **kwargs)
        elif config.strategy == RecoveryStrategy.CIRCUIT_BREAKER:
            return self._execute_with_circuit_breaker(component_name, operation, config, *args, **kwargs)
        elif config.strategy == RecoveryStrategy.FALLBACK:
            return self._execute_with_fallback(component_name, operation, config, *args, **kwargs)
        elif config.strategy == RecoveryStrategy.GRACEFUL_DEGRADATION:
            return self._execute_with_degradation(component_name, operation, config, *args, **kwargs)
        elif config.strategy == RecoveryStrategy.FAIL_FAST:
            return self._execute_fail_fast(component_name, operation, config, *args, **kwargs)
        else:
            return operation(*args, **kwargs)
    ...
    def _execute_with_retry(self,
        ...
        for attempt in range(config.max_retries + 1):
            try:
                ...
                if attempt > 0:
                    logger.info(f"Operation {component_name} succeeded on attempt {attempt + 1}")
                ...
                if attempt < config.max_retries:
                    logger.warning(f"Operation {component_name} failed on attempt {attempt + 1}, retrying in {delay}s: {e}")
                    time.sleep(delay)
                    delay *= config.backoff_multiplier
                else:
                    logger.error(f"Operation {component_name} failed after {config.max_retries + 1} attempts")
    ...
    def _execute_with_fallback(self,
        ...
        try:
            ...
            if config.fallback_fn:
                logger.warning(f"Operation {component_name} failed, using fallback: {e}")
                try:
    ...
    def _execute_with_degradation(self,
        ...
        try:
            ...
            if config.essential:
                ...
                logger.warning(f"Non-essential component {component_name} failed, continuing: {e}")
                # For non-essential components, we need to raise since we can't return None for type T
                raise
    
    def _execute_fail_fast(self,
                         component_name: str,
                         operation: Callable[..., T],
                         config: RecoveryConfig,
                         *args,
...
def with_recovery(component_name: str, 

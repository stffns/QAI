...
class AgentBuilder:
    ...
    def with_model(self, model_id: Optional[str] = None, **model_kwargs) -> AgentBuilder:
        """
        ...
        try:
            ...
            if model_id or model_kwargs:
                logger.info(f"Overriding model configuration: id={model_id}, kwargs={model_kwargs}")
    ...
    def with_tools(self, 
                   ...
                   essential_tools: Optional[List[str]] = None) -> AgentBuilder:
        """
    ...
    def with_memory(self, 
                   ...
                   memory_config: Optional[Dict[str, Any]] = None) -> AgentBuilder:
        """
        ...
        try:
            ...
            if memory_type or memory_config:
                logger.info(f"Configuring memory: type={memory_type}, config={memory_config}")
                
            self.build_config.memory = self.storage_manager.setup_storage()
            self.build_config.add_history_to_messages = self.build_config.memory is not None
            
            logger.debug(f"Memory configured: {type(self.build_config.memory).__name__ if self.build_config.memory else 'None'}")
    ...
    def with_interface(self, 
                      ...
                      markdown: Optional[bool] = None) -> AgentBuilder:
        """
        ...
        logger.debug(f"Interface configured: tool_calls={self.build_config.show_tool_calls}, markdown={self.build_config.markdown}")
        return self
    ...
    def with_validation(self, 
                       tools: bool = True,
                       memory: bool = True,
                       performance: bool = True) -> AgentBuilder:
        """
        ...
        logger.debug(f"Validation configured: tools={tools}, memory={memory}, performance={performance}")
        return self
    ...
    def validate(self) -> AgentBuilder:
        ...
        with LogStep("Validating agent configuration", "AgentBuilder"):
            ...
            if not self.build_config.instructions:
                ...
                if not self.build_config.instructions or any(not isinstance(x, str) or not x.strip() for x in self.build_config.instructions):
                    errors.append("Instructions list is empty or contains invalid items")
            elif not isinstance(self.build_config.instructions, str) or not self.build_config.instructions.strip():
            ...
            if self.build_config.memory_validation and self.build_config.memory:
                if not hasattr(self.build_config.memory, "add_memory"):
                    logger.warning("Memory object may not be compatible with Agent interface")
    ...
    def build(self) -> Agent:
        ...
        with LogStep("Building Agent instance", "AgentBuilder"):
            try:
                ...
                logger.success(f"Agent built successfully with {len(self.build_config.tools)} tools and {'streaming' if self.build_config.stream else 'non-streaming'} mode")

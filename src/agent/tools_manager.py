...
class ToolsManager:
    ...
    def _parse_tool_configs(self) -> Dict[str, ToolConfig]:
        ...
        default_tools = {
            ...
            "calculator": ToolConfig(
                ...
                essential=False,
                config={"precision": 10, "max_expression_length": 1000},
            ),
        ...
        with LogStep("Tools loading with validation", "ToolsManager"):
            ...
            for tool_name, tool_config in self.tool_configs.items():
                ...
                if result.success:
                    ...
                    logger.info(
                        f"✅ {tool_name} loaded successfully in {result.load_time:.3f}s"
                    )
                else:
            ...
            logger.info(
                f"Tools loaded: {len(self.tools)}/{len([c for c in self.tool_configs.values() if c.enabled])} successful"
            )
            return self.tools
    ...
    def _load_single_tool(self, tool_config: ToolConfig) -> ToolLoadResult:
        ...
        try:
            ...
            return ToolLoadResult(
                tool=tool, name=tool_config.name, success=True, load_time=load_time
            )
    ...
    def _load_api_tool(self, tool_config: ToolConfig) -> Any:
        ...
        try:
            ...
            from src.agent.tools.api_tools import api_health_check, api_test_endpoint
    ...
    def _load_perf_tool(self, tool_config: ToolConfig) -> Any:
        ...
        try:
            ...
            raise ToolLoadError(
                f"Performance tool {tool_config.name} not available: {e}"
            )
    ...
    def _validate_tool(self, tool: Any, tool_name: str) -> None:
        ...
        if not any(
            [
                ...
                hasattr(tool, "entrypoint")
                and callable(getattr(tool, "entrypoint")),  # Agno Function objects
            ]
        ):
            raise ToolLoadError(
                f"Tool {tool_name} doesn't implement required interface"
            )
    ...
    def get_tools_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tools_names": [
                getattr(tool, "name", tool.__class__.__name__) for tool in self.tools
            ],
            "load_results": [
                {
                    "name": result.name,
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        """
        health_report = {"overall_healthy": True, "tools": {}}

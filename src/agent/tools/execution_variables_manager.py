...
class ExecutionVariableStructure(BaseModel):
    ...
    runtime_values: Dict[str, Any] = Field(
        default_factory=dict,
        description="Current runtime values for the variables"
    )
    ...
    def get_mapping_variables(
        ...
        try:
            with self.uow_factory.create_scope() as uow:
                ...
                if not mapping:
                    logger.warning(f"No mapping found for App:{application_id}, Env:{environment_id}, Country:{country_id}")
                    return None
    ...
    def update_runtime_values(
        ...
        try:
            with self.uow_factory.create_scope() as uow:
                ...
                if not mapping:
                    logger.error(f"No mapping found for App:{application_id}, Env:{environment_id}, Country:{country_id}")
                    return False
    ...
    def initialize_execution_variables(
        ...
        try:
            with self.uow_factory.create_scope() as uow:
                ...
                logger.info(f"✅ Initialized execution_variables for mapping {mapping.id}")
                ...
                for mapping in mappings:
                    ...
                    if mapping.is_active and mapping.execution_variables:
                        ...
                        results.append({
                            ...
                            'variable_count': len(postman_vars.get('variables', {})),
                            'runtime_values_count': len(postman_vars.get('runtime_values', {}))
                        })
                
                logger.info(f"📋 Found {len(results)} mappings with execution_variables")
                return results
...
def get_execution_variables(app_id: int, env_id: int, country_id: int) -> Optional[Dict[str, Any]]:
    ...
    if success:
        ...
        if success:
            ...
            if variables:
                ...
                print(f"   Last updated: {variables.get('metadata', {}).get('last_updated')}")

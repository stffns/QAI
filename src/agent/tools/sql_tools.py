...
class QASQLTools:
    ...
    def get_table_info(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get record count
            count_result = self.sql_tools.run_sql_query(
    ...
    def explore_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(tables_raw, str):
                ...
                tables = (
                    tables_raw if isinstance(tables_raw, list) else [str(tables_raw)]
                )
            ...
            for table in tables:
                # Clean table name
                table_name = (
                    table.strip(' "[]') if isinstance(table, str) else str(table)
                )
                ...
                try:
                    ...
                    if isinstance(count_result, list) and len(count_result) > 0:
                        if (
                            isinstance(count_result[0], dict)
                            and "total" in count_result[0]
                        ):
                            record_count = count_result[0]["total"]
            ...
            return {
                ...
                "summary": {
                    ...
                    "unknown_counts": len(
                        [t for t in table_details if t.get("record_count") == "unknown"]
                    ),
                },
            }

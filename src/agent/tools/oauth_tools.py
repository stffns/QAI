...
def generate_oauth_token_raw(app_code: str, env_code: str, country_code: str) -> str:
    ...
    try:
        with LogStep(
            f"Generating OAuth token for {app_code} {env_code} {country_code}",
            "OAuthTools",
        ):
            ...
            with Session(engine) as session:
                ...
                config = oauth_repo.get_complete_oauth_config(
                    app_code, env_code, country_code
                )

                if not config:
                    logger.error(
                        f"OAuth configuration not found for {app_code} {env_code} {country_code}"
                    )
                    return json.dumps(
                        {
                            "success": False,
                ...
                try:
                    ...
                    try:
                        from src.auth.real_oauth_from_database import (
                            RealOAuthFromDatabase,
                        )
                    except Exception as import_err:
                        # Model conflict or import error
                        logger.warning(
                            f"Cannot import RealOAuthFromDatabase: {import_err}"
                        )
                        return json.dumps(
                            {
                                "success": False,
                    ...
                    logger.info(
                        f"Generating real OAuth access token for {app_code} {env_code} {country_code}"
                    )

                    # Create OAuth generator and get real token
                    oauth_generator = RealOAuthFromDatabase()
                    token_result = oauth_generator.get_real_token(
                        app_code, env_code, country_code
                    )

                    if token_result.get("access_token"):
                        logger.info(
                            f"Real OAuth access token generated successfully for {app_code} {env_code} {country_code}"
                        )
                        return json.dumps(
                            {
                                "success": True,
                                ...
                                "country_code": country_code,
                                "user_email": (
                                    config.get("user", {}).get("email")
                                    if config.get("user")
                                    else None
                                ),
                                "client_id": (
                                    config.get("client", {}).get("client_id")
                                    if config.get("client")
                                    else None
                                ),
                                "jwk_key_id": (
                                    config.get("jwk", {}).get("key_id")
                                    if config.get("jwk")
                                    else None
                                ),
                        ...
                        logger.error(
                            f"Failed to generate real OAuth token: {token_result.get('error')}"
                        )
                        return json.dumps(
                            {
                                "success": False,
...
def validate_oauth_setup_raw(app_code: str, env_code: str, country_code: str) -> str:
    ...
    try:
        with LogStep(
            f"Validating OAuth setup for {app_code} {env_code} {country_code}",
            "OAuthTools",
        ):
            # Use OAuth repository - SOLID architecture
            engine = db_manager.engine
            with Session(engine) as session:
                ...
                if not has_client:
                    missing_components.append("oauth_client")
...
🔧 **Technical Details**: {technical_error}"""
                if technical_error
                else f"❌ Token generation failed: {error_msg}"
            )
...
def oauth_validate_setup(app_code: str, env_code: str, country_code: str) -> str:
    ...
    try:
        ...
        if mapping and mapping.get("mapping_id"):
            output.append(
                f"✅ **Mapping**: ID {mapping.get('mapping_id')} - {mapping.get('app_name')}/{mapping.get('env_name')}/{mapping.get('country_name')}"
            )
        else:
        ...
        if user:
            output.append(
                f"✅ **OAuth User**: {user.get('email', 'No email')} (ID: {user.get('id')})"
            )
        else:
        ...
        if jwk:
            output.append(
                f"✅ **JWK**: Key ID {jwk.get('key_id', 'No key_id')} (ID: {jwk.get('id')})"
            )
        else:
        ...
        if client:
            output.append(
                f"✅ **OAuth Client**: {client.get('client_id', 'No client_id')} (ID: {client.get('id')})"
            )
        else:
...
__all__.extend(
    ["generate_oauth_token", "list_oauth_configurations", "validate_oauth_setup"]
)

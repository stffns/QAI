...
class ModelManager:
    ...
    def _get_validated_config(self) -> ModelConfig:
        ...
        try:
            ...
            self._model_config = ModelConfigFactory.create_config(provider, **config_data)
            
            # Validate model compatibility
            if not validate_model_compatibility(self._model_config):
                raise ModelManagerError(f"Model configuration not compatible: {self._model_config.id}")
    ...
    def _build_openai_chat(self, config: ModelConfig) -> Any:
        ...
        allowed_fields = {
            "id", "api_key", "base_url", "organization", "project",
            "temperature", "max_tokens", "timeout", "seed", "response_format"
        }
        
        # Convert to dict and filter allowed fields
        config_dict = config.model_dump()
        kwargs = {k: v for k, v in config_dict.items() if k in allowed_fields and v is not None}

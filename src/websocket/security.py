...
class SecurityManager:
    ...
    async def authenticate_token(self, token: Optional[str]) -> Optional[str]:
        ...
        try:
            with LogStep("JWT token validation", "SecurityManager"):
                ...
                # Check cache first
                if (
                    token in self.auth_cache
                    and self.cache_expiry.get(token, 0) > time.time()
                ):
                ...
                # Cache the result
                self.auth_cache[token] = {"user_id": user_id, "payload": payload}
    ...
    async def check_rate_limit(self, identifier: str) -> bool:
        ...
        if (
            current_time - self.rate_limit_cleanup_last
            > self.config.rate_limiting.cleanup_interval
        ):
    ...
    def _get_jwt_secret(self) -> str:
        ...
        if not secret or secret.startswith("${"):
            ...
            secret = (
                os.getenv("WEBSOCKET_JWT_SECRET")
        ...
        if not secret or secret.startswith("${"):
            ...
            self.logger.warning(
                "Using generated JWT secret - configure WEBSOCKET_JWT_SECRET for production"
            )
    ...
    async def _cleanup_old_rate_limit_windows(self) -> None:
        ...
        expired_tokens = [
            token
            for token, expiry in self.cache_expiry.items()
        ...
        if identifiers_to_remove or expired_tokens:
            self.logger.debug(
                f"Cleaned up {len(identifiers_to_remove)} rate limit windows and {len(expired_tokens)} auth cache entries"
            )
...
class CORSManager:
    ...
    def __init__(self, cors_config):
        ...
        self.allowed_origins = (
            set(cors_config.origins) if cors_config.origins else set()
        )
        self.allow_all_origins = "*" in self.allowed_origins

        self.logger.info(
            f"CORS manager initialized with {len(self.allowed_origins)} allowed origins"
        )
    ...
    def check_origin(self, origin: Optional[str]) -> bool:
        ...
        if not origin:
            self.logger.debug(
                "Allowing connection without Origin header (development mode)"
            )
            return True

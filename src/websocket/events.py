...
class ChatMessagePayload(BasePayload):
    ...
    language: Optional[str] = Field(
        None, description="Código de idioma (ej: 'en', 'es')"
    )
    locale: Optional[str] = Field(
        None, description="Código de localización (ej: 'en-US', 'es-ES')"
    )
    timestamp: Optional[str] = Field(
        None, description="Timestamp del cliente (ISO string)"
    )
...
class WebSocketEnvelope(BaseModel):
    ...
    version: ProtocolVersion = Field(
        default=ProtocolVersion.V2_0, description="Versión del protocolo"
    )
    id: str = Field(
        default_factory=lambda: str(uuid.uuid4()), description="ID único del mensaje"
    )
    ts: int = Field(
        default_factory=lambda: int(time.time() * 1000), description="Timestamp en ms"
    )
    ...
    correlation_id: Optional[str] = Field(
        None, description="ID de correlación para trazabilidad"
    )
...
class WebSocketEnvelopeFactory:
    ...
    def create_agent_response_stream_chunk(
        ...
        payload = AgentResponseStreamChunkPayload(
            chunk=chunk, chunk_index=chunk_index, is_complete=is_complete
        )
        return WebSocketEnvelope(
    ...
    def create_system_event(
        ...
        payload = SystemEventPayload(
            event_name=event_name, description=description, severity=severity, data=data
        )
        return WebSocketEnvelope(
    ...
    def create_connection_event(
        ...
        payload = ConnectionEventPayload(
            event_type=event_type, client_info=client_info, session_data=session_data
        )
        return WebSocketEnvelope(
    ...
    def create_health_check(
        ...
        payload = HealthCheckPayload(status=status, metrics=metrics)
        return WebSocketEnvelope(
...
def create_envelope_from_legacy_event(
...
if __name__ == "__main__":
    ...
    if parsed_envelope.is_chat_message():
        ...
        if isinstance(original_chat_payload, ChatMessagePayload) and isinstance(
            parsed_chat_payload, ChatMessagePayload
        ):
            print(
                f"Content matches: {original_chat_payload.content == parsed_chat_payload.content}"
            )

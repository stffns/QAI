...
class QAAgentAdapter:
    ...
    def __init__(
        self,
        user_id: str = "websocket_user@qai.com",
    ...
    def _initialize_agent(self):
        ...
        try:
            ...
            model_manager, tools_manager, storage_manager = (
                self._create_component_managers(config, managers)
            )
            ...
            interface_config, instructions, reasoning_config = (
                self._get_configuration_sections(config)
            )
            ...
            if self.enable_reasoning and reasoning_config.get("enabled", False):
                self._add_reasoning_capabilities(
                    agent_args, reasoning_config, openai_chat
                )
    ...
    def _setup_project_paths(self) -> str:
        ...
        )  # Go up 3 levels: websocket -> src -> QAI
    ...
    def _import_agent_components(self, project_root: str):
        ...
        try:
            ...
            manager_modules = [
                ...
                "storage_manager",
                "config.models",
                "config",  # Clear config modules too
            ]
    ...
    def _prepare_base_agent_args(
        self, model, instructions, tools, storage, interface_config
    ):
        ...
        logger.info(
            f"🔧 Interface config show_tool_calls: {interface_config.get('show_tool_calls', 'NOT_FOUND')}"
        )
        ...
        agent_args = {
            ...
            "show_tool_calls": websocket_config.get(
                "show_tool_calls", interface_config.get("show_tool_calls", False)
            ),
            "markdown": websocket_config.get(
    ...
    async def chat(self, message: str) -> str:
        ...
        try:
            ...
            if self.response_manager:
                ...
                logger.info(
                    f"🎯 Response strategy: {strategy.name} (model: {context.model}, max_tokens: {context.max_tokens})"
                )
    ...
    async def process_message(
        ...
        try:
            ...
            logger.info(
                f"🔄 Processing message with context: {', '.join(context_info)}"
            )
                ...
                try:
                    ...
                    for chunk in response:
                        ...
                        if hasattr(chunk, "content") and chunk.content:
                            ...
                            if chunk_str and not any(
                                tech_term in chunk_str
                                for tech_term in [
                    ...
                    if response_parts:
                        ...
                        logger.info(
                            f"✅ Message processed successfully: {len(result)} characters (from generator)"
                        )
                        return result
                    else:
                        logger.warning(
                            "⚠️ Generator was empty or contained only technical events"
                        )
            ...
            if hasattr(response, "content") and response.content:
                result = str(response.content)
                logger.info(
                    f"✅ Message processed successfully: {len(result)} characters"
                )
                return result
            elif hasattr(response, "text") and response.text:
                result = str(response.text)
    ...
    async def process_message_stream(
        ...
        try:
            ...
            logger.info(
                f"🔄 Processing STREAMING message with context: {', '.join(context_info)}"
            )
            logger.info(f"💬 Message: {message[:100]}...")
            ...
            try:
                ...
                try:
                    ...
                    for chunk in response_iter:
                        ...
                        if (
                            hasattr(chunk, "event")
                            and chunk.event == "RunResponseContent"
                        ):
                            if hasattr(chunk, "content") and chunk.content:
                        ...
                        if chunk_content:
                            ...
                            if is_markdown_block_complete(content_buffer):
                                ...
                                await asyncio.sleep(
                                    0.05
                                )  # Slightly longer delay for better formatting
    ...
    def _apply_dynamic_model_config(self, context: Dict[str, Any]):
        ...
        try:
            ...
            current_model = (
                getattr(self.agent.model, "id", None)
            ...
            if recommended_model and current_model != recommended_model:
                logger.info(
                    f"🔄 Switching model: {current_model} → {recommended_model}"
                )

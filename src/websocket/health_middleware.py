...
try:
    from scripts.check_openai_health import (
        check_openai_health,
        get_status_message_for_user,
    )
except ImportError:
    import os
    import sys
...
class HealthAwareErrorHandler:
    ...
    async def enhance_error_with_health_info(
        ...
        might_be_openai_related = any(
            keyword in error_text_lower for keyword in error_keywords
        )
        ...
        if might_be_openai_related:
            ...
            if health_info:
                ...
                elif status == "unknown" and not health_info.get(
            ...
            enhanced_error["user_message"] = (
                "This appears to be a technical issue not related to OpenAI services."
            )
...
async def enhance_websocket_error(
    ...
    """
    return await health_aware_error_handler.enhance_error_with_health_info(

...
class PerformanceMiddleware(BaseMiddleware):
    ...
    async def process(self, context: MiddlewareContext) -> bool:
        ...
        self.metrics["average_processing_time"] = (
            ...
            / self.metrics["connections_processed"]
        )
...
class WebSocketMiddleware:
    ...
    def _build_middleware_pipeline(self, security_manager) -> Optional[BaseMiddleware]:
        ...
        if security_manager:
            ...
            if self.config.security.rate_limiting.enabled:
                rate_limit_middleware = RateLimitMiddleware(
                    self.config.security.rate_limiting, security_manager
                )
        ...
        self.logger.info(
            f"Built middleware pipeline with {len(middlewares)} middlewares"
        )
        return middlewares[0] if middlewares else None

    async def process_connection(self, websocket: ServerConnection, path: str) -> bool:
        """
        ...
        try:
            with LogStep("WebSocket middleware processing", "WebSocketMiddleware"):
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Could not extract IP address from websocket: {e}"
                    )
                    ip_address = "unknown"
                except Exception as e:
    ...
    async def process_envelope(
        ...
        try:
            ...
            if hasattr(self.config.logging, "log_envelopes") and getattr(
                self.config.logging, "log_envelopes", False
            ):
                self.logger.debug(
            ...
            self.logger.debug(
                f"Envelope processing traceback: {traceback.format_exc()}"
            )
            return None
...
if __name__ == "__main__":
    ...
    async def test_middleware():
        ...
        security_config = SecurityConfig(
            ...
            cors=CorsConfig(enabled=True, origins=["http://localhost:3000"]),
        ...
        print(
            f"Middleware pipeline created with metrics: {middleware.middleware_metrics}"
        )

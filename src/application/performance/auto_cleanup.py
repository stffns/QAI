...
try:
    ...
    from database.models.performance_test_executions import PerformanceTestExecution  # type: ignore
...
class PerformanceAutoCleanup:
        ...
        try:
            ...
            if keep_recent is None:
                try:
                    ...
                    if config_path.exists():
                        ...
                        keep_recent = config.get("performance", {}).get("auto_cleanup", {}).get("keep_recent", 1)
                    else:
                        keep_recent = 1
                except Exception:
                    keep_recent = 1
            ...
            try:
                ...
                if config_path.exists():
                    with open(config_path) as f:
                        config = yaml.safe_load(f)
                    if not config.get("performance", {}).get("auto_cleanup", {}).get("enabled", True):
            ...
            logger.info(f"🧹 Starting post-execution cleanup (keeping {keep_recent} recent)")
            ...
            logger.info(f"📦 Post-execution cleanup completed: {archived_count} executions archived")
    ...
    def pre_execution_cleanup(self, keep_recent: Optional[int] = None) -> None:
        ...
        Args:
            keep_recent: Number of recent executions to keep in perf_runs.
        """
        logger.warning("🔄 pre_execution_cleanup is deprecated, use post_execution_cleanup")
    ...
    def _verify_data_persistence(self, execution_id: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"⚠️  Could not verify data persistence for {execution_id}: {e}")
            return False
            ...
            for item in self.archive_dir.iterdir():
                if item.is_dir() and item.stat().st_mtime < cutoff_time:
                    try:
                        ...
                        logger.warning(f"⚠️  Could not delete old archive {item.name}: {e}")
            
            if deleted_count > 0:
                logger.info(f"🗑️  Cleaned {deleted_count} old archives (>{retention_days} days)")

...
class ModelConfig(BaseModel):
    ...
    id: str = Field(
        default_factory=lambda: _env_str("MODEL_ID")
        or _env_str("AGENT_DEFAULT_MODEL")
        or "gpt-5-mini",
...
class DatabaseConfig(BaseModel):
    ...
    host: Optional[str] = Field(
        default_factory=lambda: os.environ.get("DB_HOST"), description="Database host"
    )
    port: Optional[int] = Field(
        default_factory=lambda: _env_int("DB_PORT"), description="Database port"
    )
    name: Optional[str] = Field(
        default_factory=lambda: os.environ.get("DB_NAME"), description="Database name"
    )
    ...
    enable_migrations: bool = Field(
        default_factory=lambda: os.environ.get("DB_ENABLE_MIGRATIONS", "true").lower()
        == "true",
        description="Enable automatic database migrations",
    )
    backup_enabled: bool = Field(
        default_factory=lambda: os.environ.get("DB_BACKUP_ENABLED", "false").lower()
        == "true",
...
class ToolConfig(BaseModel):
    ...
    settings: Dict[str, Any] = Field(
        default_factory=dict, description="Tool-specific configuration settings"
    )
    required_permissions: List[str] = Field(
        default_factory=list, description="Required permissions for tool execution"
    )
...
class ToolsConfig(BaseModel):
    ...
    enabled: bool = Field(
        default_factory=lambda: os.environ.get("TOOLS_ENABLED", "true").lower()
        == "true",
    ...
    safety_mode: bool = Field(
        default_factory=lambda: os.environ.get("TOOLS_SAFETY_MODE", "true").lower()
        == "true",

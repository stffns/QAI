...
class LoggingConfig(BaseModel):
    ...
    enable_file_logging: bool = Field(
        default_factory=lambda: os.environ.get("LOG_ENABLE_FILE", "true").lower()
        == "true",
    ...
    enable_console_logging: bool = Field(
        default_factory=lambda: os.environ.get("LOG_ENABLE_CONSOLE", "true").lower()
        == "true",
    ...
    enable_json_logs: bool = Field(
        default_factory=lambda: os.environ.get("LOG_ENABLE_JSON", "false").lower()
        == "true",
    ...
    performance_log_file: str = Field(
        default_factory=lambda: os.environ.get(
            "LOG_PERFORMANCE_FILE", "logs/performance.log"
        ),
        description="Performance log file path",
    )

    enable_audit_logging: bool = Field(
        default_factory=lambda: os.environ.get("LOG_ENABLE_AUDIT", "false").lower()
        == "true",
    ...
    capture_exceptions: bool = Field(
        default_factory=lambda: os.environ.get("LOG_CAPTURE_EXCEPTIONS", "true").lower()
        == "true",
        description="Automatically capture and log unhandled exceptions",
    )

    include_traceback: bool = Field(
        default_factory=lambda: os.environ.get("LOG_INCLUDE_TRACEBACK", "true").lower()
        == "true",
    ...
    def validate_log_level(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v_upper not in LogLevel.all_levels():
            ...
            raise ValueError(
                f"Invalid log level: {v}. " f"Must be one of: {valid_levels}"
            )
        return v_upper
    ...
    def validate_logging_configuration(self):
        ...
        if self.console_level:
            ...
            if level_order.index(self.console_level) < level_order.index(self.level):
                import warnings
